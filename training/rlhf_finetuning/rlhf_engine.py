...
class DeepSpeedRLHFEngine():
    ...
    def __init__(self, actor_model_name_or_path, critic_config_name_or_path, critic_model_name_or_path, 
        ...
        self.critic = self._init_critic(config_name_or_path=critic_config_name_or_path, critic_model_name_or_path=critic_model_name_or_path)
        self.reward = self._init_reward(config_name_or_path=critic_config_name_or_path, critic_model_name_or_path=critic_model_name_or_path)
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        optim_params = get_optimizer_grouped_parameters(actor_model, self.args.actor_weight_decay)
        ...
        # print("output model params", id(actor_model))
        # for i, p in enumerate(actor_model.transformer.h):
        #     print(torch.distributed.get_rank(), i, p.attn.q_proj.weight.shape, id(p.attn.q_proj.weight))
    ...
    def _init_ref(self, actor_model_name_or_path):
        ...
        ds_config['train_micro_batch_size_per_gpu'] = self.args.per_device_mini_train_batch_size
        ds_config['train_batch_size'] = self.args.per_device_mini_train_batch_size * torch.distributed.get_world_size() * self.args.gradient_accumulation_steps_actor
    ...
    def _init_ema(self, actor_model_name_or_path):
        ...
        if zero_stage != 3:
            ...
            zero_stage = 0
        ds_config = get_eval_ds_config(self.args.offload_reference_model, zero_stage)
        actor_model_ema = create_hf_model(AutoModelForCausalLM, actor_model_name_or_path, self.tokenizer, ds_config)
        if self.args.actor_lora_dim > 0:
            actor_model_ema = convert_linear_layer_to_lora(actor_model_ema, self.args.actor_lora_module_name, self.args.actor_lora_dim)
        ema_engine, *_ = deepspeed.initialize(model=actor_model_ema, config=ds_config)
        log_init("EMA", stime=stime)
        return ema_engine

    def _init_critic(self, config_name_or_path, critic_model_name_or_path):
        stime = log_init("Critic")
        ds_config = get_train_ds_config(offload=self.args.offload, stage=self.args.critic_zero_stage)
        ds_config['train_micro_batch_size_per_gpu'] = self.args.per_device_mini_train_batch_size
        #TODO(jeff): we should probably set grad accumlation steps here as well for clarity
        ds_config['train_batch_size'] = self.args.per_device_mini_train_batch_size * torch.distributed.get_world_size() * self.args.gradient_accumulation_steps
        ...
        critic_model = create_critic_model(
            ...
            tokenizer=self.tokenizer_critic if self.tokenizer_critic else self.tokenizer,
        ...
        optim = AdamOptimizer(optim_pararms, lr=self.args.critic_learning_rate, betas=(0.9, 0.999))
    ...
    def _init_reward(self, config_name_or_path, critic_model_name_or_path):
        ...
        critic_model = create_critic_model(
            ...
            ds_config=ds_eval_config,
            rlhf_training=True)
        reward_engine, *_ = deepspeed.initialize(model=critic_model, config=ds_config)
